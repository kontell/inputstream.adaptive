...
// numBytes still includes the trailing start code (3 or 4 bytes), stripped here.
bool ES_hevc::AppendExtraData(int buf_ptr, unsigned int numBytes)
{
  if (numBytes < 4)
    return false;

  unsigned int nalSize = numBytes - 3;
  if (es_buf[buf_ptr + numBytes - 4] == 0)
    nalSize = numBytes - 4;
  ...
  ed[0] = ed[1] = ed[2] = 0;
  ed[3] = 1;
  memcpy(ed + 4, es_buf + buf_ptr, nalSize);
